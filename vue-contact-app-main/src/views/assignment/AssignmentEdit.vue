<template>
  ...
  <h4>{{assignmentInfo.assignment_name}}</h4>
  <hr />
  <div class="row">
    <div class="col-md-4">
...
export default class AssignmentEdit extends Vue {
  ...
  courses: List<ISimpleCourseResult> = new List<ISimpleCourseResult>()
  categories: List<ISimpleCategoryResult> = new List<ISimpleCategoryResult>()
  ...
  assignmentInfo: ISimpleAssignmentResult = {}
  errorMessages: List<string> = new List<string>()

  async mounted()
      : Promise<void> {
    console.log("mounted", "AssignmentEdit");
    this.assignmentInfo = await this.assignmentService.getInfo(Number(this.id))
    this.courses = await this.courseService.getAllSimple()
    this.categories = await this.categoryService.getSimpleByCourse(this.assignmentInfo.course_id)

    this.assignmentName = this.assignmentInfo.assignment_name
    this.assignmentComment = this.assignmentInfo.assignment_comment
    this.weight = this.assignmentInfo.assignment_weight
    this.maxPoints = this.assignmentInfo.assignment_max_points
    this.inCourse = this.assignmentInfo.course_id
    this.inCategory = this.assignmentInfo.category_id_list.at(-1)
  }
  ...
  : Promise<void> {
    ...
    if (this.errorMessages.length <= 0) {
      await this.assignmentService.updateAssignment(Number(this.id), Number(this.inCategory), Number(this.inCourse),
          this.assignmentName, this.assignmentComment, Number(this.maxPoints), Number(this.weight))
